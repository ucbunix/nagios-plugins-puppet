...
UOM=s   # default to secondss
CRITICAL_THRESHOLD= # no critical threshold by default
WARNING_THRESHOLD= # no warning threshold by default
...
if [[ $UOM =~ ^m\(inute\)\? ]]; then
  CRITICAL_THRESHOLD=$($EXPR ${CRITICAL_THRESHOLD} \* 60)
  WARNING_THRESHOLD=$($EXPR ${WARNING_THRESHOLD} \* 60)
fi

if [[ "$UOM" =~ ^h\(our\)? ]]; then 
  CRITICAL_THRESHOLD=$($EXPR ${CRITICAL_THRESHOLD} \* 60 \* 60)
  WARNING_THRESHOLD=$($EXPR ${WARNING_THRESHOLD} \* 60 \* 60)
fi

if [[ "$UOM" =~ ^d\(ay\)? ]]; then
  CRITICAL_THRESHOLD=$($EXPR ${CRITICAL_THRESHOLD} \* 60 \* 60 \* 24)
  WARNING_THRESHOLD=$($EXPR ${WARNING_THRESHOLD} \* 60 \* 60 \* 24)
fi
...
SINCE_LASTRUN=$( $EXPR $(date +%s) - $(cat /var/lib/puppet/state/last_run_summary.yaml|grep last_run|awk '{ print $2 }' ) )
